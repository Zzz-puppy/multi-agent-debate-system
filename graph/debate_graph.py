...
class DebateOrchestrator:
    ...
    def run_debate(self, topic: str, max_rounds: int = 3) -> DebateState:
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            state["round_scores"].append(RoundEvalEntry(
                round=round_num, pro_scores=pro_eval, con_scores=con_eval,
            ))
        print(f"\n[教师总结]")
        final_scores = aggregate(state["round_scores"])
        state["scores"] = {"pro": final_scores["pro_score"], "con": final_scores["con_score"]}
        state["winner"] = final_scores["winner"]
        summary = self.teacher.summarize_debate(
            topic, max_rounds, state["conversation_history"],
            final_scores["pro_score"], final_scores["con_score"], final_scores["winner"],
        )
        state["teacher_summary"] = summary
        winner_text = {"pro": "正方", "con": "反方", "平局": "平局"}.get(final_scores["winner"], final_scores["winner"])
        print(f"\n{'='*50}\n  最终评分\n{'='*50}")
        for side, label in [("pro_details", "正方"), ("con_details", "反方")]:
            details = final_scores[side]
            print(f"  {label}:")
            for key, cname in {"logic_score":"逻辑性","evidence_score":"论据质量","rebuttal_score":"反驳能力","expression_score":"语言表达"}.items():
                print(f"    {cname}: {details.get(key, 0)}/10")
            print(f"    总分: {sum(details.get(k, 0) for k in ['logic_score','evidence_score','rebuttal_score','expression_score']) * 0.25:.1f}")
        print(f"\n  正方: {final_scores['pro_score']}分 | 反方: {final_scores['con_score']}分")
        print(f"  获胜方: {winner_text}\n{'='*50}\n")
        self._archive_debate(state)
        self.pro_agent.profile.update_after_debate(topic, final_scores["pro_score"], summary, [], [])
        self.con_agent.profile.update_after_debate(topic, final_scores["con_score"], summary, [], [])
        return state
    ...
    def _archive_debate(self, state: DebateState) -> None:
        ...
        self.teacher.long_term.store_debate(
            topic=state["topic"], summary=state["teacher_summary"] or "",
            pro_arguments=pro_args, con_arguments=con_args,
            teacher_score={"pro": state["scores"]["pro"],"con": state["scores"]["con"],"winner": state["winner"]},
